...
function loadEarnParticipantSelect(){
    ...
    document.getElementById("submit-participant-earn-btn").addEventListener("click", function(){event.preventDefault();earnParticipantLookup(0)});
}

function loadSpendParticipantSelect(){
    document.querySelector('#biblebucks-content').innerHTML=bb_participantSpendTemplate;
    document.getElementById("home-btn").addEventListener("click", loadBibleBucksApp);
    document.getElementById("submit-participant-spend-btn").addEventListener("click", function(){event.preventDefault();spendParticipantLookup(0)});
}
